 ...
 * PHP QR Code encoder
 *
 * This file contains a MERGED version of PHP QR Code library.
 * It was auto-generated from full version for your convenience,
 * and edited after-the-fact for use in YOURLS-Integrated-QRCode.
 *
 *       		(0‿0)
 *
 * THIS merged version has been stripped of reduntant information,
 * was configured to make use of an external cache, has enabled error 
 * loging, and uses the strong mask matching. It seeks to strike a 
 * ballance between file compression/tidyness, performance, and
 * quality.
 *
 * For the full version, docs, and examples of use please visit:
 *
 *    http://phpqrcode.sourceforge.net/
 *    https://sourceforge.net/projects/phpqrcode/
 *
 * Based in part on libqrencode C library distributed under LGPL 2.1
...
/*
 * Config file. Edited for Inline QRCode for YOURLS
 *
 */
    ...
    define('QR_LOG_DIR', $me);			// error logs dir
    define('QR_FIND_BEST_MASK', true);		// true = +++ mask quality / - speed
    define('QR_FIND_FROM_RANDOM', false);	// false = check all avl masks; or number of random mask checks
    define('QR_DEFAULT_MASK', 2);		// if QR_FIND_BEST_MASK === false
    define('QR_PNG_MAXIMUM_SIZE', 1024);	// max png pixel width (GD and PHP strain)
 ...
 */
    ...
    class QRtools {
        ...
        public static function log($outfile, $err)
        {
            if (QR_LOG_DIR !== false) {
                if ($err != '') {
                    if ($outfile !== false) {
                        file_put_contents(QR_LOG_DIR.basename($outfile).'-errors.txt', date('Y-m-d H:i:s').': '.$err.PHP_EOL, FILE_APPEND);
                    } else {
                        file_put_contents(QR_LOG_DIR.'errors.txt', date('Y-m-d H:i:s').': '.$err.PHP_EOL, FILE_APPEND);
                    }
                }    
            }
        }
	...
	public static function position($position=" ", $objSize1, $objSize2) {
		...
		switch($position) {
			default:
			case "center":
				$p['x'] = ($objSize1[0] - $objSize2[0]) / 2;
				$p['y'] = ($objSize1[1] - $objSize2[1]) / 2;
			break;
			case "topleft":
				$p['x'] = 0;
				$p['y'] = 0;
 ...
 */
	...
	private static function addLogo(&$image) {
		...
		$transparencyIndex = imagecolortransparent($logo);
		$transparencyColor = array('red' => 255, 'green' => 255, 'blue' => 255);
		if ($transparencyIndex >= 0) {$transparencyColor = imagecolorsforindex($logo, $transparencyIndex);}
		$transparencyIndex    = imagecolorallocatealpha($image, $transparencyColor['red'], $transparencyColor['green'], $transparencyColor['blue'], 127);
		...
		$logo_qr_width = $QR_width*$s;
		$scale = $logo_width/$logo_qr_width;
		$logo_qr_height = $logo_height/$scale;

		$position = QRTools::position($position, array($QR_width, $QR_height), array($logo_qr_width, $logo_qr_height));

		imagecopyresampled($image, $logo, $position['x'], $position['y'], 0, 0, $logo_qr_width, $logo_qr_height, $logo_width, $logo_height);
	}
    ...
    class QRencode {
        ...
        public function encodeSVG($intext, $outfile = false,$saveandprint=false) 
        {
            try {
                ...
                $maxSize = (int)(QR_PNG_MAXIMUM_SIZE / (count($tab)+2*$this->margin));
//                QRvect::svg($tab, $outfile, min(max(1, $this->size), $maxSize), $this->margin,$saveandprint);
	...
	private static function addLogo(&$src) {
		...
		$logo = substr($logo, strpos($logo, "<svg "));
		$logo = str_replace(array("<svg", "</svg>"), array("<g", "</g>"), $logo);

		$FOO=$logo = simplexml_load_string($logo);
		$xml = simplexml_load_string($src);

		##### scaling
		$QR_width = $xml["width"];
		$QR_height = $xml["height"];

		$viewBox = explode(" ", $logo["viewBox"]);
		$logo_width = $viewBox[2];
		$logo_height = $viewBox[3];

		$scale = min($QR_width/$logo_width, $QR_height/$logo_height)*$s;
		...
		$logo["transform"]="translate(".$position['x']." ".$position['y'].") scale(".(float) $scale.")";
		unset($logo["width"]);
		unset($logo["height"]);

		$domdict = dom_import_simplexml($xml);
		$domcat  = dom_import_simplexml($logo);
		$domcat  = $domdict->ownerDocument->importNode($domcat, TRUE);
		$domdict->appendChild($domcat);

		$src = $domdict->ownerDocument->saveXML($domdict);
		return $src;
	}
        //----------------------------------------------------------------------
